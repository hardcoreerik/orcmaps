...
| go-pmtiles CLI | 1.28.2 (`protomaps/go-pmtiles` tag `v1.28.2`, commit `5898b17`), BSD-3-Clause | Host provisioning only: extract bbox/GeoJSON regions from an existing PMTiles archive via `tools/pack-builder/build_regional_pack.py` | No — separately installed CLI, not vendored, linked, or used at device runtime. The builder records the actual supplied version in each manifest. |
