...
| Viewport/camera | `include/orcmap/viewport.hpp`, `src/core/viewport.cpp` | Implemented: center/zoom/pan, visible bounds, `FitBounds` (frame a pack) and `FillBounds` (fill the screen), `MinFillZoom` (query the fill zoom without moving the camera, so an application can derive a zoom-out floor from what it has installed), projection, anchored zoom, visible-tile enumeration, wrapped X, clamped Y. Centre and zoom are derived from the caller's viewport size, so no per-board constants are needed. Integer zoom; no overzoom. |
